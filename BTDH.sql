...
BEGIN
    ...
    ELSEIF
    NEW.score > 10 THEN SET NEW.score = 10;
	END IF;
END // 
...
DROP PROCEDURE IF EXISTS tg_prevent_pass_update;
...
FOR EACH ROW
BEGIN
    IF OLD.score >= 4.00 THEN
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'Lỗi nghiệp vụ: Sinh viên đã qua môn (Điểm >= 4.0), không được phép sửa điểm!';
    END IF;
END //
